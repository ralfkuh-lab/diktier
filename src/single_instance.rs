...
#[derive(Debug)]
pub enum Acquire {
    /// Sperre gehört uns, solange der [`FileLock`] lebt.
    Held(FileLock),
...
impl Acquire {
    pub fn held(self) -> Option<FileLock> {
        match self {
...
/// Gehaltene Sperre. Der Lock endet mit dem Schließen des Deskriptors, also
/// beim `Drop` — die Datei selbst bleibt bewusst liegen (Löschen wäre ein
/// Rennen gegen den nächsten Starter, der sie schon geöffnet hat).
#[derive(Debug)]
pub struct FileLock {
    _file: File,
    path: PathBuf,
...
/// Die gehaltene Single-Instance-Sperre: **alle** nutzbaren Orte auf einmal.
#[derive(Debug)]
...
impl InstanceLock {
    pub fn paths(&self) -> impl Iterator<Item = &Path> {
        self.locks.iter().map(FileLock::path)
    }
...
/// Kern der Instanzsperre, gegen Temp-Pfade testbar.
fn acquire_all(
    candidates: &[PathBuf],
    on_problem: &mut dyn FnMut(String),
) -> Result<InstanceAcquire, LockError> {
...
pub fn try_lock(path: &Path) -> Result<Acquire, LockError> {
    ...
    match flock_exclusive_nonblocking(&file) {
        Ok(true) => Ok(Acquire::Held(FileLock {
            _file: file,
            path: path.to_path_buf(),
        })),
...
/// Windows nimmt laut §5.3 einen per-user Named Mutex. Phase 3d ist Linux;
/// hier steht der `cfg`-Stub wie im übrigen Repo — er sperrt **nicht**.
#[cfg(windows)]
